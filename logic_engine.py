...
class HybridBrain:
    ...
    async def _neural_scan(self, text):
        ...
        try:
            completion = await self.groq_client.chat.completions.create(
                ...
                messages=[
                    {"role": "system", "content": "Analyze for Sri Lankan Business. If text is junk/chatter set validity=false. Return JSON: {validity, score, reason, sentiment_type, logistics_status, location_name, lat, lon}"},
    ...
    def _fallback_symbolic_scan(self, text):
        ...
        score = 0
        if "dead" in text_lower or "died" in text_lower or "killed" in text_lower: score += 50
    ...
    async def analyze(self, text):
        ...
        if ai_reason == "Neural Error":
             final_score = self._fallback_symbolic_scan(text)
